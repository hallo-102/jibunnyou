...
def _fallback_score(df: pd.DataFrame, cfg: dict) -> pd.Series:
    ...
    for col in df.columns:
        ...
        used += 1
    if used == 0:
        implied = 1.0 / pd.to_numeric(df["win_odds"], errors="coerce").clip(lower=1.01)
...
def _try_lightgbm_predict(df: pd.DataFrame, cfg: dict, project_root: Path) -> pd.Series | None:
    ...
    if missing:
        return None
    x = df[names].apply(pd.to_numeric, errors="coerce").fillna(0.0)
    return pd.Series(booster.predict(x), index=df.index, dtype=float)
